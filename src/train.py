...
class FixedEnvironment(MultiAgentGridWorld):
    ...
    def step(self, actions):
        ...
        if collision:
            ...
            # REDUCED penalty
            rewards = [-2.0] * self.num_agents  # Was -10, now -2
        else:
            # Update positions
            for idx, (agent, new_pos) in enumerate(zip(self.agents, new_positions)):
                ...
                if agent.has_item:
                    target = self.location_B
                else:
                    target = self.location_A
                ...
                if new_dist < old_dist:
                    rewards[idx] += 1.0  # NEW: Reward progress
                elif new_dist > old_dist:
                    rewards[idx] -= 0.5  # Small penalty for moving away
                
                # Pickup
                if agent.position == self.location_A and not agent.has_item:
                    agent.pickup_item()
                    rewards[idx] += 5.0  # Increased from 1.0
                
                # Delivery
                elif agent.position == self.location_B and agent.has_item:
                    agent.dropoff_item()
                    self.total_deliveries += 1
                    rewards[idx] += 20.0  # Increased from 10.0
                
                # Tiny step penalty
                rewards[idx] -= 0.05  # Reduced from 0.1
...
class TrainingLogger:
    ...
    def plot_training_curves(self, save_path=None):
        ...
        fig.suptitle('Training Metrics - FIXED VERSION', fontsize=16, fontweight='bold')
        
        # Rewards
        axes[0, 0].plot(self.episode_rewards, alpha=0.6)
        axes[0, 0].plot(self._moving_average(self.episode_rewards, 50), linewidth=2)
        ...
        # Deliveries
        axes[0, 1].plot(self.episode_deliveries, alpha=0.6)
        axes[0, 1].plot(self._moving_average(self.episode_deliveries, 50), linewidth=2)
        ...
        # Collisions
        axes[0, 2].plot(self.episode_collisions, alpha=0.6)
        axes[0, 2].plot(self._moving_average(self.episode_collisions, 50), linewidth=2)
        ...
        # Steps
        axes[1, 0].plot(self.episode_steps, alpha=0.6)
        axes[1, 0].plot(self._moving_average(self.episode_steps, 50), linewidth=2)
        ...
        axes[1, 1].plot(self._moving_average(self.episode_losses, 50), linewidth=2)
        ...
        # Epsilon
        axes[1, 2].plot(self.epsilon_history, linewidth=2)
    ...
    def save_metrics(self, filepath):
        metrics = {
            ...
            'best_deliveries': self.best_deliveries
        }
    ...
    def save_improved_config():
        config = {
            'environment': {
                ...
                'num_agents': 4,
                'max_steps': 1500,
                'max_collisions': 10  # Increased from 4
            },
            'training': {
                'episodes': 1000,
...
class ImprovedTrainer:
    ...
    def __init__(self, config_path="config_improved.yaml"):
        ...
        self.logger = TrainingLogger(log_dir=f"logs/fixed_run_{timestamp}")
        
        self.num_episodes = self.config['training']['episodes']
        self.target_update_freq = 500  # Update more frequently
        self.save_freq = 100
    ...
    def train(self):
        print("\n" + "="*70)
        print("🚀 IMPROVED TRAINING - WITH ALL FIXES")
        print("="*70)
        print(f"Episodes: {self.num_episodes}")
        print(f"Max Collisions: {self.env.max_collisions}")
        print(f"Starting Positions: CORNERS (spread out)")
        ...
        for episode in range(1, self.num_episodes + 1):
            ...
            avg_loss = np.mean(self.agent_system.agents[0].losses[-episode_steps:]) if self.agent_system.agents[0].losses else 0
    ...
    def _save_checkpoint(self, episode):
        ...
        filepath = f"models/fixed_dqn_episode_{episode}.pth"
        self.agent_system.save(filepath)
        if episode != 'final':
    ...
    def evaluate(self, num_episodes=20):
        ...
        for ep in range(num_episodes):
            ...
            print(f"Eval {ep+1:2d}: Deliveries={self.env.total_deliveries}, "
                  f"Collisions={self.env.total_collisions}, "
                  f"Reward={total_reward:.1f}")
        
        print("\n" + "="*70)
        print("EVALUATION RESULTS")
        print("="*70)
        print(f"Avg Deliveries: {np.mean(eval_deliveries):.2f} ± {np.std(eval_deliveries):.2f}")
        print(f"Avg Collisions: {np.mean(eval_collisions):.2f} ± {np.std(eval_collisions):.2f}")
        print(f"Avg Reward:     {np.mean(eval_rewards):.2f} ± {np.std(eval_rewards):.2f}")
        
        success_rate = np.mean(eval_deliveries) / max(1, np.mean(eval_deliveries) + np.mean(eval_collisions))
...
def main():
    ...
    print("="*70)
    print("\nKey Improvements:")
    print("✅ Starting positions: Spread out (corners)")
    print("✅ Reduced collision penalty: -2 (was -10)")
    print("✅ Added progress rewards: +1 for moving closer")
    print("✅ Increased delivery reward: +20 (was +10)")
    print("✅ Slower epsilon decay: 0.998 (was 0.995)")
    print("✅ Lower learning rate: 0.0005 (was 0.001)")
    print("✅ More collision budget: 10 (was 4)")
    print("="*70 + "\n")
    ...
    print("\n🎉 All done! Check logs/ folder for results")
